...
/* ECDH, AES GCM in encrypt message data and file data and decrypt using post method */
...
/*  Sender Key Protocol which generate single key for a group */
...
export class ChatGateway {
    ...
    private users: { [userId: string]: { sockets: string[] } } = {};
    private groups: { [groupId: string]: { members: string[], senderKey?: string } } = {};

    // Encrypt using AES-GCM with the Sender Key
    private encryptData(data: Buffer, secret: string): { encryptedData: string, iv: string, authTag: string } {
        const iv = crypto.randomBytes(12);
        const cipher = crypto.createCipheriv('aes-256-gcm', Buffer.from(secret, 'hex'), iv);
        const encrypted = Buffer.concat([cipher.update(data), cipher.final()]);
    ...
    // Decrypt using AES-GCM with the Sender Key
    private decryptData(encryptedData: string, iv: string, authTag: string, secret: string): Buffer {
        const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(secret, 'hex'), Buffer.from(iv, 'hex'));
        decipher.setAuthTag(Buffer.from(authTag, 'hex'));
        return Buffer.concat([decipher.update(Buffer.from(encryptedData, 'hex')), decipher.final()]);
    ...
    // Register a user and store their socket connections
    @SubscribeMessage('registerUser')
    registerUser(@MessageBody() userId: string, @ConnectedSocket() client: Socket) {
        if (!this.users[userId]) {
            this.users[userId] = { sockets: [client.id] };
    ...
    // Create a group and generate the Sender Key
    ...
    createGroup(@MessageBody() data: { groupId: string, members: string[] }, @ConnectedSocket() client: Socket) {
        ...
        // Generate a single Sender Key (AES-256) for the group
        const senderKey = crypto.randomBytes(32).toString('hex');
        ...
        // Send the Sender Key securely to all group members
        data.members.forEach(userId => {
            this.users[userId]?.sockets.forEach(socketId => {
                this.server.to(socketId).emit('senderKey', { groupId: data.groupId, senderKey });
            });
        });

        console.log(`Group ${data.groupId} created with Sender Key: ${senderKey}`);
    }

    // Send an encrypted message to the group
    @SubscribeMessage('sendMessageToGroup')
    sendMessageToGroup(@MessageBody() data: { groupId: string, senderId: string, message: string }) {
        const group = this.groups[data.groupId];
        if (!group || !group.senderKey) {
            console.error(`Group ${data.groupId} not found or sender key missing.`);
            return;
        }

        // Encrypt the message with the group's Sender Key
        const encrypted = this.encryptData(Buffer.from(data.message), group.senderKey);

        // Send the encrypted message to all group members
        ...
        console.log(`Encrypted message sent in group ${data.groupId}`);
    }

    // Handle user leaving a group (optional: rekeying)
    ...
    leaveGroup(@MessageBody() data: { userId: string, groupId: string }) {
        ...
        // Optional: Rotate the Sender Key when a user leaves
        if (group.members.length > 0) {
            group.senderKey = crypto.randomBytes(32).toString('hex');

            // Notify remaining users about the new Sender Key
            group.members.forEach(userId => {
                this.users[userId]?.sockets.forEach(socketId => {
                    this.server.to(socketId).emit('senderKey', { groupId: data.groupId, senderKey: group.senderKey });
                });
            });

            console.log(`Sender Key rotated for group ${data.groupId}`);
        } else {
...
/* Sender Key Protocol which generate single key for a group (only message data)*/
...
export class ChatGateway {
    ...
    private groups: { 
        [groupId: string]: { 
            members: string[], 
            senderKeys: { [senderId: string]: string } // Store Sender Key per sender
        } 
    } = {};
    ...
    registerUser(@MessageBody() userId: string, @ConnectedSocket() client: Socket) {
        ...
        } else if (!this.users[userId].sockets.includes(client.id)) {
            this.users[userId].sockets.push(client.id);
    ...
    // Create a group (no sender key is generated at this point)
    @SubscribeMessage('createGroup')
    createGroup(@MessageBody() data: { groupId: string, members: string[] }, @ConnectedSocket() client: Socket) {
        if (this.groups[data.groupId]) {
            client.emit('senderKey', { message: 'Group already exists' });
        ...
        this.groups[data.groupId] = {
            members: data.members,
            senderKeys: {} // Empty at first, sender keys are generated by each sender
        };
    ...
    sendMessageToGroup(@MessageBody() data: { groupId: string, senderId: string, message: string }) {
        ...
        // Generate a Sender Key for the sender if they don't have one yet
        if (!group.senderKeys[data.senderId]) {
            group.senderKeys[data.senderId] = crypto.randomBytes(32).toString('hex');

            // Send this new Sender Key to all group members
            group.members.forEach(userId => {
                this.users[userId]?.sockets.forEach(socketId => {
        ...
        // Encrypt the message using the sender's own Sender Key
        const senderKey = group.senderKeys[data.senderId];
        const encrypted = this.encryptData(Buffer.from(data.message), senderKey);
        ...
        console.log(`Encrypted message sent in group ${data.groupId} by ${data.senderId}`);
    ...
    leaveGroup(@MessageBody() data: { userId: string, groupId: string }) {
        ...
        // Remove user from group
        group.members = group.members.filter(id => id !== data.userId);
        delete group.senderKeys[data.userId]; // Remove the sender's key

        console.log(`User ${data.userId} left group ${data.groupId}`);

        // Optional: Rotate all remaining users' Sender Keys
        if (group.members.length > 0) {
            group.members.forEach(senderId => {
                ...
                // Notify all users about the updated Sender Keys
                group.members.forEach(userId => {
                    this.users[userId]?.sockets.forEach(socketId => {
                        this.server.to(socketId).emit('senderKey', { 
                            groupId: data.groupId, 
...
/* Sender Key Protocol which generate single key for a group (message data and file data) (without filter null properties)*/
/*
import {
    WebSocketGateway,
    WebSocketServer,
...
@WebSocketGateway({ cors: true, maxHttpBufferSize: 1e8 }) // Allow large file transfers
export class ChatGateway {
    @WebSocketServer()
    server: Server;
...
sendMessageToGroup(
    @MessageBody() 
    data: { groupId: string; senderId: string; message?: string; filePath?: string }
) {
    const group = this.groups[data.groupId];
    if (!group) {
        console.error(`Group ${data.groupId} not found.`);
    ...
    // **Step 1: Generate or Retrieve Sender Key**
    if (!group.senderKeys[data.senderId]) {
        group.senderKeys[data.senderId] = crypto.randomBytes(32).toString('hex');
        
        // Send the sender key to all members
        group.members.forEach(userId => {
            this.users[userId]?.sockets.forEach(socketId => {
                this.server.to(socketId).emit('senderKey', {
                    groupId: data.groupId,
    ...
    // **Step 2: Encrypt the Message (if exists)**
    if (data.message) {
        encryptedMessage = this.encryptData(Buffer.from(data.message), senderKey);
    }

    // **Step 3: Encrypt the File (if exists)**
    if (data.filePath) {
        try {
            const fileBuffer = fs.readFileSync(data.filePath);
    ...
    // **Step 4: Send Encrypted Data to All Group Members**
    group.members.forEach(userId => {
        this.users[userId]?.sockets.forEach(socketId => {
            this.server.to(socketId).emit('receiveMessage', {
                groupId: data.groupId,
                senderId: data.senderId,
                encryptedMessage: encryptedMessage ? encryptedMessage.encryptedData : null,
                messageIv: encryptedMessage ? encryptedMessage.iv : null,
                messageAuthTag: encryptedMessage ? encryptedMessage.authTag : null,
                encryptedFile: encryptedFile ? encryptedFile.encryptedData : null,
                fileIv: encryptedFile ? encryptedFile.iv : null,
                fileAuthTag: encryptedFile ? encryptedFile.authTag : null,
...
/* Sender Key Protocol which generate single key for a group (message data and file data) */
...
export class ChatGateway {
    ...
    @SubscribeMessage('sendMessageToGroup')
    sendMessageToGroup(@MessageBody() data: { groupId: string; senderId: string; message?: string; filePath?: string }) {
        const group = this.groups[data.groupId];
        if (!group) {
            console.error(`Group ${data.groupId} not found.`);
            return;
